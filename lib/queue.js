...
function fetchRepo(workerObj, done) {
  /**
    ...
    also we have to save names in workerObj, but for now fake stuff
  **/
  workerObj.repoZipPath = path.join(workerObj.tmpPath, 'llzip.zip');
  fs.writeFileSync(workerObj.repoZipPath, fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/llzip.zip'));
  done(null, workerObj);
}

function fetchTestCases(workerObj, done) {
  /**
    Here goes fetching test cases for assignment, but for now, fake stuff
  **/
  var expectedOutput = fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/referenceOutput.txt');
  workerObj.tests = [
  ...
  fs.writeFileSync(path.join(workerObj.tmpPath, 'IndexListTester.java'), fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/IndexListTester.java'));
  fs.writeFileSync(path.join(workerObj.tmpPath, 'IndexListTester2.java'), fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/IndexListTester2.java'));
  fs.writeFileSync(path.join(workerObj.tmpPath, 'IndexListTester3.java'), fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/IndexListTester3.java'));
  fs.writeFileSync(path.join(workerObj.tmpPath, 'IndexListTester4.java'), fs.readFileSync('/home/daniel/workspace/Lab4 - Linked List/src/IndexListTester4.java'));
  done(null, workerObj);
}
...
function postprocess(workerObj, done) {
  /**
    Do some post processing of the data, like, interpret data
  **/
  done(null, workerObj);
}
