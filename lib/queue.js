...
function preprocess(submission) {
  return function (done) {
    ...
    workerObj.submitDate = (new Date()).toISOString();
  ...
  async.waterfall([
    ...
    function (isSingleFile, _done) {
      ...
      if (isSingleFile) {
        ...
        writeStream = fs.createWriteStream(workerObj.repoZipPath);
        projectParams = {
          'username': workerObj.user.lastName + workerObj.user.firstName,
...
function fetchTestCases(workerObj, done) {
  ...
  async.map(workerObj.tests, function (testCase, next) {
    ...
    //Total socre sum
    totalScore = totalScore + parseInt(testCase.score);
...
function veredict(test, workerObj) {
  return function (safeOutBuff, javaOutBuff, done) {
    ...
    async.every(_.range(N), function (i, callback) {
      ...
      if(result) {
        workerObj.score = workerObj.score + parseInt(test.score);
...
function postprocess(workerObj, done) {
  ...
  database.getDB().collection('submissions').insert(workerObj, function (err) {
    if (err) {
      workerObj.err = err;
      workerObj.errCode = DB_FAIL;
      return done(workerObj);
...
function queueWorker(submission, callback) {
  async.waterfall([
    ...
    if (err) {
      ...
      //Not a sucesfull run, we still need to save submission into db
      err.finalVerdict = err.status;
      err.status = 'Done';
      database.getDB().collection('submissions').insert(err, function (err) {
        if (err) {
