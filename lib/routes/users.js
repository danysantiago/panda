...
function createUser(req, res, next) {
  ...
  async.waterfall([
    ...
    function (user, callback) {
      ...
      //TODO (Daniel): Check between student and prof
      //Something about project limit...
      var gitlabUser = {
        'email': userObj.email,
    ...
    function (gitRes, gitBody, callback) {
      if (gitRes && gitRes.statusCode === 201) {
        ...
        //Gitlab user created, insert new user into DB
        req.users.insert(userObj, callback);
      } else {
        res.send(400, {'error': 'Error creating Gitlab repo'});
        return;
      }
    }
