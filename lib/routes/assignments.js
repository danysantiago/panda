    ...
    async.nextTick(function(){
      ...
      async.waterfall([
        ...
        function (students, callback) {
          ...
          async.each(students, function (student, done) {
            ...
            var projectParams = {
              ...
              'username': utils.emailToUsername(student.email)
            };

            async.waterfall([
              function (_done) {
                //Create repo first
                gitlab.project.create(projectParams, _done);
              },

              function (gitRes, gitBody, _done) {
                if(gitRes && gitRes.statusCode === 201) {

                  var repoInfo = {
                    'url': gitBody.http_url_to_repo,
                    'id': gitBody.id,
                    'assigId': assign._id,
                  ...
                  req.users.insertRepo(student._id, repoInfo, _done);
                } else {
                  req.log.error('Unable to create assignment repo');
                }
              },

              function (updatedDoc, updateInfo, _done) {
        ...
        function (callback) {
          ...
          var emailMsg ='The following assignment has been created for the ' + 
           'course ' + course.code + ' - ' + course.name + '.\n' +
            '\nAssignment Name: ' + assign.name +
            '\n\nDeadline: ' + assign.deadline +
            '\n\nNumber of Tries: ' + assign.numOfTries +
            '\n\nTo view more details head to www.pandacode.sytes.net'; 

          var email = {
            'to': emailList,
            'from': 'noreply@pandacode.sytes.net',
            'fromname': 'Panda Code System',
            'subject': 'New Assignment on ' + course.code,
            'text': emailMsg
          };
...
function updateAssignment(req, res, next) {
  ...
  if(req.body.name) {
    res.send(400, {'error': 'Assignment name is not editable'});
    return;
  }
  ...
  async.waterfall([
    ...
    function (callback) {
      req.assignments.update(assignId, assignObj, callback);
    }

  ], function (err, oldAssignment) {
...
function createTestCase(req, res, next) {
  ...
  if(!req.body.score) {
    res.send(400, {'error': 'Invalid payload, must have score'});
    return;
  }

  testCase.score = parseInt(req.body.score, 10);

  console.log(testCase);

  //Validate time limit
  if(req.body.timeLimit) {
    testCase.timeLimit = parseInt(req.body.timeLimit, 10);
  }
  
  //Validate mem limit
  if(req.body.memLimit) {
    testCase.memLimit = parseInt(req.body.memLimit, 10);
  }
  ...
  if (testerFile) {
    ...
    default:
      res.send(400, {'error':
        'Invalid payload, testerFile file must .java'});
      return;
    }
  } else {
    testCase.type = 'I/O';
  ...
  async.waterfall([
    ...
    function (testInput, callback) {
      testCase.resource.input = testInput.toString();
      callback();
    },
    ...
    function (testOutput, callback) {
      testCase.resource.output = testOutput.toString();
      callback();
    },
...
middleware.post('/assignments', express.bodyParser({
