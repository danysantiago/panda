...
function createAssignment(req, res, next) {
  ...
  **********/
  ...
  if (req.files && req.files.instructions) {
    ...
    if (instructions.headers['content-type'] === 'application/pdf') {
      instructionsType = 'file';
    } else if (instructions.headers['content-type'] === 'text/plain') {
    ...
    function (callback) {
      if (instructions && instructionsType === 'file') {
        ...
        fs.readFile(instructions.path, function (err, data) {
          ...
          grid.put(data, {
            'content-type': instructions.headers['content-type'], 
            'metadata': {
