...
test.use(function (req, res, next) {
  var tmpPath = utils.generateTmpPath(path.join(config.root,'jail/tmp'));
...
test.post('/jsubmit', express.bodyParser(bodyParserOptions),
    function (req, res) {
  ...
  var jailTmpDir = path.join('tmp', tmpDirSplit[tmpDirSplit.length-1]);
  ...
  //Runtime limits
  var procLimit = 20; // (>= 12)
  var memLimit = 20480; //Kilobytes
  var fileLimit = 32768; //Kilobytes

  var jvmMemLimit = 20; //Megabytes (=> 2)
  var timeLimit = 3; //Seconds
  ...
  function (err, safeExecBuff, javaOutBuff, compileTime) {
    ...
    if (executeStats.status !== 'OK') { //Not OK, lets check what happened...
      //If java finished with error we gather stack, otherwise a limit was
      //exceeded
      if (executeStats.status.indexOf('Command exited with ' + 
          'non-zero status') !== -1) {
    ...
    veredictOutput(javaOutBuff, testOutput, function (veredict) {
      ...
      //Delete tmp folder after 3 seconds
