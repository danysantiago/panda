...
function updateCourse(req, res, next) {
  ...
  req.courses.update(courseId, req.body, function (err, result) {
    if(err) {
      return next(err);
    }
...
function broadcastEmail(req, res, next) {
  var courseId = req.params.courseId;
  ...
  async.waterfall([
    ...
    function (callback) { //Get students
      req.users.findStudentsSimple(course.Users, function (err, users) {
        course.users = users; //Set users array for response
        callback(err);
      });
    },

    function (callback) { //Get graders
      req.users.findGraders(course.Graders, function (err, graders) {
        course.graders = graders; //Set users array for response
        callback(err);
      });
    },
    ...
    function (callback) {
      var emailList = [];
      async.each(course.users, function (student, done) {
    ...
    function (emailList, callback) {
      var grader = course.graders[0];
      //Send email to student about assignment
      var email = {
        'to': emailList,
        'from': grader.email,
        'fromname': grader.firstName + ' ' + grader.lastName,
...
middleware.post('/courses/:courseId/broadcast', express.bodyParser(),
  broadcastEmail);
