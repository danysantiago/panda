...
Projects.prototype = {
	...
	*/
	'populate': function(params, callback) {
		...
		var instr = __dirname + '/../createRepo.sh ';
		instr += params.name.toLowerCase() + ' root aguacate ';
		instr += params.username + ' ' + params.archive; 

		exec(instr, function (error, stdout, stderr) {
			if(self.logObj) {
				self.logObj.debug(stdout);
				self.logObj.debug(stderr);
			}
